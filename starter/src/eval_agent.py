...
# APPEND (not prepend): src/ must keep priority for data_loader (the learner copy
# lacks map_diagnosis_to_triage); sahayak_starter exists only in learner/.
_learner_dir = str(Path(__file__).resolve().parent.parent / "learner")
if _learner_dir not in sys.path:
    sys.path.append(_learner_dir)
...
from sahayak_starter import (
    ...
    ensure_disclaimer,
    escalation_floor,  # used by the loop-closure probe below (was missing from imports)
...
MODEL_ID = "hermes3:8b"
# Decoding config (system parameter, not harness logic): near-greedy sampling for
# structured-output compliance on the 8B local model. All metric logic unchanged.
